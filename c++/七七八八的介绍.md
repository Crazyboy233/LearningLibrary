...
static_cast<T&&>
```
换句话说：std::move = 强制把左值变成右值引用

**问：移动什么时候发生以及结果是什么？**

移动行为发生在：**移动构造函数，移动赋值运算符**
举个例子：
```cpp
std::string s1 = "hello";
...
1. std::move(s1) 把 s1 转成右值引用
2. std::string 的移动构造函数被调用
3. s2 “偷走”了 s1 的内部 buffer
4. s1 变成一个“有效但未定义内容”的状态

关键点：
如果类没有定义移动构造函数，std::move 也没用，只会退化成拷贝。

### std::mt19937

所属头文件：`random`

std::mt19937 是 C++11 标准引入的高性能伪随机数生成器，全称是 Mersenne Twister（梅森旋转算法），是目前 C++ 开发中最常用、最推荐的随机数生成器。

- mt19937：生成 32 位无符号整数 的随机数
- mt19937_64：生成 64 位无符号整数 的随机数

### std::uniform_real_distribution

所属头文件：`random`

**C++ 生成均匀分布浮点数随机数**的标准工具，和 `std::mt19937` 搭配使用，专门解决**生成指定范围小数**的需求。

`std::uniform_real_distribution` 是**均匀实数分布器**：

- 作用：把 `mt19937` 生成的大整数，**映射成你指定区间内的均匀随机浮点数**
- 特点：区间内**每个数被选中概率完全相等**（均匀分布）
- 类型：`float / double / long double` 都支持

```cpp
// 生成 [a, b) 左闭右开区间的随机浮点数
std::uniform_real_distribution<T> 分布名(最小值, 最大值);
```

- 默认区间：**`[a, b)`**（包含最小值，不包含最大值）
