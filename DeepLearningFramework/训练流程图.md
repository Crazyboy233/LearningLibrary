...
这部分是在数据准备阶段做的，和框架无关：
- 原始日志清洗
- 特征拼接、统计（用户7日点击数、item曝光次数等）
- 数值归一化、分桶
- 生成训练样本文件（tfrecord / parquet / csv）

**第二层：在线特征处理（在模型内部，forward 里）**
```
数据文件（tfrecord / csv / parquet）
    ↓
DataLoader / Dataset 读取，按 batch 喂入
    ↓
进入 forward
    ├─ Embedding 查表（离散ID → 向量）
    ├─ 特征拼接 torch.cat
    ├─ 特征交叉（如 FM、DIN 的 attention）
    └─ MLP 等网络结构
    ↓
输出 logit
    ↓
算 loss → backward → 更新参数
```
也就是说，从样本输入到训练中之后，再对特征处理（类似特征交叉，特征拼接等）是发生在 forward 里的，算是 forward 的一部分