...
GRPO 是 DeepSeek 团队于2024年2月在论文《DeepSeekMath: Pushing the Limits of Mathematical Reasoning in Open Language Models》中提出的方法，专门为**推理型任务**（如数学、代码）设计。它是 PPO 的改进版，核心改动有两点：
...
PPO（Proximal Policy Optimization，近端策略优化）是强化学习中最经典的策略梯度算法之一，由 OpenAI 在 2017 年提出，也是 RLHF 中训练语言模型的核心算法。

PPO 中有四个核心网络模型，分别是：

- Actor Model（策略模型）：输入 Prompt，生成 Response（Rollout）更新参数，让生成结果越来越好。
- Reference Model（参考模型，ref）：Reference 是 **Actor 的初始快照**。不参与训练。作用仅为计算 KL Divergence（KL 散度）。
