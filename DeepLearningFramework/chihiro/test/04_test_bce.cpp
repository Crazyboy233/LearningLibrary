...
// 编译命令
// g++ -std=c++17 ./core/*.cc ./test/04_test_bce.cpp && ./a.out
...
int main() {
    ...
    for (int epoch = 0; epoch < EPOCHS; ++epoch) {
        ...
        auto logits = fc2.forward(h);   // 原始输出
        ...
        auto& lv = logits->value();
        std::vector<double> prob(lv.size());
        for (size_t i = 0; i < lv.size(); ++i) {
            prob[i] = 1.0 / (1.0 + std::exp(-lv[i]));
        }
        // if ((epoch + 1) % 100 == 0) {
