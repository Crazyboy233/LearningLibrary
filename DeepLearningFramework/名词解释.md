...
KTO（Kahneman-Tversky Optimization）由 Ethayarajh 等人于 2024 年提出，基于诺贝尔经济学奖得主 Kahneman 和 Tversky 的**前景理论（Prospect Theory）**，将人类对损失和收益的非对称感知引入 LLM 对齐训练。
...
RLHF 奠定了对齐训练的基本范式，之后的方法都是在其基础上做简化或改进：DPO 把阶段二和三合并成一个监督学习步骤；GRPO 保留强化学习框架但去掉 Critic；Constitutional AI（Claude 所用）则在偏好标注环节引入 AI 辅助。核心问题始终是同一个：如何把人类关于"好"的判断有效地传递给模型。
